...
from sam3d_objects.utils.memory_utils import offload_model, delete_model, clear_memory, print_memory_usage
...
class InferencePipelinePointMap(InferencePipeline):
    ...
    def compute_pointmap(self, image, pointmap=None):
        ...
        if pointmap is None:
            ...
            pointmaps = output["pointmaps"].float()  # Cast to float32 for pytorch3d compatibility
    ...
    def run(
        ...
        offload_after_use=True,  # Memory optimization: offload models after each stage
    ) -> dict:
        image = self.merge_image_and_mask(image, mask)
        with self.device: 
            print_memory_usage("Before depth model")
            pointmap_dict = self.compute_pointmap(image, pointmap)
            pointmap = pointmap_dict["pointmap"]
            pts = type(self)._down_sample_img(pointmap)
            pts_colors = type(self)._down_sample_img(pointmap_dict["pts_color"])

            # === MEMORY OPTIMIZATION: Offload depth model (not needed anymore) ===
            if offload_after_use and hasattr(self, 'depth_model') and self.depth_model is not None:
                # MoGe is a wrapper, offload its internal model
                if hasattr(self.depth_model, 'model'):
                    offload_model(self.depth_model.model, "depth_model.model")
                self.depth_model = None
                clear_memory()
                print_memory_usage("After depth model offload")
            ...
            print_memory_usage("Before Stage 1 (SS)")
            ss_return_dict = self.sample_sparse_structure(
                ss_input_dict,
            ...
            # === MEMORY OPTIMIZATION: Offload Stage 1 models ===
            if offload_after_use:
                if "ss_generator" in self.models:
                    offload_model(self.models["ss_generator"], "ss_generator")
                if "ss_decoder" in self.models:
                    offload_model(self.models["ss_decoder"], "ss_decoder")
                if "ss_condition_embedder" in self.condition_embedders:
                    offload_model(self.condition_embedders["ss_condition_embedder"], "ss_condition_embedder")
                clear_memory()
                print_memory_usage("After Stage 1 offload")
            ...
            print_memory_usage("Before Stage 2 (SLAT)")
            ...
            # === MEMORY OPTIMIZATION: Offload Stage 2 generator ===
            if offload_after_use:
                if "slat_generator" in self.models:
                    offload_model(self.models["slat_generator"], "slat_generator")
                if "slat_condition_embedder" in self.condition_embedders:
                    offload_model(self.condition_embedders["slat_condition_embedder"], "slat_condition_embedder")
                clear_memory()
                print_memory_usage("After Stage 2 offload")

            print_memory_usage("Before decoding")
