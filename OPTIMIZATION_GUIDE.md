...
```bash
# Segmentation (SAM) and depth (MoGe) run on the GPU via MPS
# (Metal Performance Shaders) for PyTorch. The 3-D reconstruction
# stages (sparse structure, SLAT, mesh decode) run on the CPU.
# This split is automatic; no flags are required.
```
