...
class SparseMultiHeadAttention(nn.Module):
    ...
    def _linear(
        ...
        if isinstance(x, SparseTensor):
            feats = x.feats
            # Move weights to input device (MPS GPU) for GPU acceleration
            if module.weight.device != feats.device:
                weight = module.weight.to(device=feats.device, dtype=feats.dtype)
                bias = module.bias.to(device=feats.device, dtype=feats.dtype) if module.bias is not None else None
                result = F.linear(feats, weight, bias)
            else:
                result = module(feats)
            return x.replace(result)
        else:
            # Move weights to input device for GPU acceleration
            if module.weight.device != x.device:
