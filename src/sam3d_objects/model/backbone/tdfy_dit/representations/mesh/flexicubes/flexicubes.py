...
# Mock kaolin.utils.testing.check_tensor for CPU compatibility
# This utility just validates tensor shapes and is not strictly required
def check_tensor(tensor, shape, throw=True):
    """Mock of kaolin.utils.testing.check_tensor for CPU compatibility."""
    if not torch.is_tensor(tensor):
        return throw and False
...
class FlexiCubes:
    def __init__(self, device="cpu"):  # Changed from cuda for CPU compatibility
        ...
        self.cube_corners_idx = torch.pow(2, torch.arange(8, device=device, requires_grad=False))
    ...
    def __call__(self, voxelgrid_vertices, scalar_field, cube_idx, resolution, qef_reg_scale=1e-3,
        ...
        # Dynamically update device to match input tensors for MPS compatibility
        # This ensures all internal operations use the correct device
        device = voxelgrid_vertices.device
        self.device = device
        
        # Move all internal lookup tables to new device (lazy migration from CPU)
        if self.dmc_table.device != device:
            self.dmc_table = self.dmc_table.to(device)
            self.num_vd_table = self.num_vd_table.to(device)
            self.check_table = self.check_table.to(device)
            self.tet_table = self.tet_table.to(device)
            self.quad_split_1 = self.quad_split_1.to(device)
            self.quad_split_2 = self.quad_split_2.to(device)
            self.quad_split_train = self.quad_split_train.to(device)
            self.cube_corners = self.cube_corners.to(device)
            self.cube_corners_idx = self.cube_corners_idx.to(device)
            self.cube_edges = self.cube_edges.to(device)
            self.edge_dir_table = self.edge_dir_table.to(device)
            self.dir_faces_table = self.dir_faces_table.to(device)
            self.adj_pairs = self.adj_pairs.to(device)
        
        # Move all input tensors to same device for MPS compatibility
        ...
        if gamma_f is not None:
            gamma_f = gamma_f.to(device)
        if voxelgrid_colors is not None:
            voxelgrid_colors = voxelgrid_colors.to(device)
    ...
    def _normalize_weights(self, beta, alpha, gamma_f, surf_cubes, weight_scale):
        ...
        """
        # Use input tensor device for MPS compatibility
        device = surf_cubes.device
        n_cubes = surf_cubes.shape[0]

        if beta is not None:
            beta = (torch.tanh(beta) * weight_scale + 1)
        else:
            beta = torch.ones((n_cubes, 12), dtype=torch.float, device=device)

        if alpha is not None:
            alpha = (torch.tanh(alpha) * weight_scale + 1)
        else:
            alpha = torch.ones((n_cubes, 8), dtype=torch.float, device=device)

        if gamma_f is not None:
            gamma_f = torch.sigmoid(gamma_f) * weight_scale + (1 - weight_scale) / 2
        else:
            gamma_f = torch.ones((n_cubes), dtype=torch.float, device=device)
    ...
    def _get_case_id(self, occ_fx8, surf_cubes, res):
        ...
        """
        # Use input tensor device for MPS compatibility (self.device may be CPU)
        device = occ_fx8.device
        case_ids = (occ_fx8[surf_cubes] * self.cube_corners_idx.to(device).unsqueeze(0)).sum(-1)

        problem_config = self.check_table.to(device)[case_ids]
        ...
        problem_config_full = torch.zeros(list(res) + [5], device=device, dtype=torch.long)
        ...
        idx = torch.arange(case_ids.shape[0], device=device)[to_check][within_range][to_invert]
    ...
    def _identify_surf_edges(self, scalar_field, cube_idx, surf_cubes):
        ...
        """
        # Use input tensor device for MPS compatibility
        device = cube_idx.device
        occ_n = scalar_field < 0
        all_edges = cube_idx[surf_cubes][:, self.cube_edges.to(device)].reshape(-1, 2)
        # torch.unique with dim is not implemented on MPS, use CPU fallback
        if all_edges.device.type == "mps":
            all_edges_cpu = all_edges.cpu()
            unique_edges, _idx_map, counts = torch.unique(all_edges_cpu, dim=0, return_inverse=True, return_counts=True)
            unique_edges = unique_edges.to(device)
            _idx_map = _idx_map.to(device)
    ...
    def _linear_interp(self, edges_weight, edges_x):
        ...
        """
        # Use input tensor device for MPS compatibility
        device = edges_weight.device
        edge_dim = edges_weight.dim() - 2
        assert edges_weight.shape[edge_dim] == 2
        edges_weight = torch.cat([torch.index_select(input=edges_weight, index=torch.tensor(1, device=device), dim=edge_dim), -
                                 torch.index_select(input=edges_weight, index=torch.tensor(0, device=device), dim=edge_dim)]
                                 , edge_dim)
    ...
    def _compute_vd(self, voxelgrid_vertices, surf_cubes_fx8, surf_edges, scalar_field,
        ...
        # Use input tensor device for MPS compatibility
        device = voxelgrid_vertices.device
        alpha_nx12x2 = torch.index_select(input=alpha, index=self.cube_edges.to(device), dim=1).reshape(-1, 12, 2)
        ...
        total_num_vd = 0
        vd_idx_map = torch.zeros((case_ids.shape[0], 12), dtype=torch.long, device=device, requires_grad=False)
        ...
        for num in torch.unique(num_vd):
            ...
            curr_edge_group_to_vd = torch.arange(
                curr_num_vd, device=device).unsqueeze(-1).repeat(1, 7) + total_num_vd
            total_num_vd += curr_num_vd
            curr_edge_group_to_cube = torch.arange(idx_map.shape[0], device=device)[
        ...
        vd = torch.zeros((total_num_vd, 3), device=device)
        beta_sum = torch.zeros((total_num_vd, 1), device=device)
        ...
        '''
        if voxelgrid_colors is not None:
            vd_color = torch.zeros((total_num_vd, C), device=device)
        ...
        v_idx = torch.arange(vd.shape[0], device=device)  # + total_num_vd
    ...
    def _triangulate(self, scalar_field, surf_edges, vd, vd_gamma, edge_counts, idx_map, vd_idx_map, surf_edges_mask, training, vd_color):
        ...
        if not training:
            ...
            faces[mask] = quad_vd_idx[mask][:, self.quad_split_1.to(quad_vd_idx.device)]
            faces[~mask] = quad_vd_idx[~mask][:, self.quad_split_2.to(quad_vd_idx.device)]
            ...
            vd_center_idx = torch.arange(vd_center.shape[0], device=vd.device) + vd.shape[0]
            vd = torch.cat([vd, vd_center])
            faces = quad_vd_idx[:, self.quad_split_train.to(vd.device)].reshape(-1, 4, 2)
