...
class Dino(torch.nn.Module):
    def __init__(
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                if 'http' in error_str or 'timeout' in error_str or 'connection' in error_str or '504' in error_str:
                    ...
                    if os.path.exists(local_repo):
                        ...
                        logger.info("Successfully loaded DINO from local cache")
                    else:
                        raise RuntimeError(f"Network unavailable and no local cache found at {local_repo}") from e
                else:
                    raise
    ...
    def _preprocess_input(self, x):
        ...
        if self.normalize_images:
            # Ensure mean/std are on same device as input (defensive for MPS)
            mean = self.mean.to(device=_resized_images.device, dtype=_resized_images.dtype)
            std = self.std.to(device=_resized_images.device, dtype=_resized_images.dtype)
