...
### Removed — upstream Metal reconstruction kernels; reconstruction is now CPU-only

The custom Metal reconstruction kernels inherited from the upstream port
([`ZimengXiong/Sam3D-Objects-MLX`](https://github.com/ZimengXiong/Sam3D-Objects-MLX))
were **removed** from the repository, because that upstream project ships with no
license (all rights reserved) and its code was therefore not ours to
redistribute. With those components gone, the project **no longer contains
upstream-owned code** and can be published under its own license (first-party
code under AGPL-3.0; `sam3d_objects/` under Meta's SAM License).

#### Removed
- `sparse_conv.metal` and `conv_metal.py` (Metal sparse-convolution kernel and wrapper).
- `flash_attn.metal` and `metal_flash_attn.py` (Metal flash-attention kernel and wrapper).
- All `metal` sparse-backend / `metal_fa` attention selection paths and the
  `SPARSE_BACKEND` / `SPARSE_ATTN_BACKEND` / `SAM3D_FORCE_CPU` environment toggles.

#### Changed
- **Reconstruction now runs on the CPU** through a pure-PyTorch sparse path
  (naive gather/scatter sparse convolution) with `sdpa` attention. Segmentation
  (SAM) and depth (MoGe) continue to run on the GPU via PyTorch MPS.
- The CPU/MPS split is now automatic and unconditional; there is no backend flag
  to set.

#### Notes
- Reconstruction is somewhat slower on heavy objects (roughly ~20% on large
  meshes) than the removed GPU path. Segmentation and depth are unaffected.
- If an explicit upstream license grant is obtained later, the optional GPU
  reconstruction path can return as an add-on.
...
- Renamed the recommended conda environment from `sam-3d-mlx` to **`sam-3d`** and
  ...
  framework (it uses PyTorch MPS, not MLX); the old name was
