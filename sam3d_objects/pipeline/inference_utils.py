...
def downsample_sparse_structure(
    ...
    Args:
        ...
        force: when True, always downsample regardless of the input coord count
            (used to force the fast, low-memory decode path for every object).
        subsample_cap: ceiling used for the post-downsample random subsample. Defaults
            to max_coords. Decoupled so a caller can force the factor-2 downsample while
            still keeping the int32-safe ceiling (42000) as the subsample limit.

    Returns:
        Downsampled coord_batch with coordinates rescaled if downsampling is needed
    """
    if subsample_cap is None:
        subsample_cap = max_coords
    if not force and coord_batch.shape[0] <= max_coords:
    ...
    if unique_combined.shape[0] > subsample_cap:
