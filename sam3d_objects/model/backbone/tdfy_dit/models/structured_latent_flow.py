...
class SLatFlowModel(nn.Module):
    ...
    def forward(
        ...
        # Stability: Run transformer blocks on CPU if on MPS to avoid MatrixMult crashes.
        # For 25k voxels, CPU is very fast and much more stable.
        device = h.feats.device
        if device.type == "mps":
            # Move transformer part to CPU
            self.blocks.to("cpu")
            self.blocks.apply(convert_module_to_f32)
            
            h_cpu_feats = h.feats.to("cpu").float()
            h_cpu = h.replace(h_cpu_feats)
            t_emb_cpu = t_emb.to("cpu").float()
            cond_cpu = cond.to("cpu").float()
            
            for block in self.blocks:
                h_cpu = block(h_cpu, t_emb_cpu, cond_cpu)
            
            # Move back to original device
            h = h_cpu.to(device).type(self.dtype)
