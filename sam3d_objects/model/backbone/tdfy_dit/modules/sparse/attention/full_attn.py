...
if ATTN == "xformers":
    ...
    # Metal Flash Attention with fallback to SDPA
    try:
        from .metal_flash_attn import metal_masked_sdpa as masked_sdpa
        print("[ATTENTION] Using Metal Flash Attention")
    except ImportError:
        from .masked_sdpa import masked_sdpa
        print("[ATTENTION] Metal unavailable, using SDPA fallback")
else:
...
def sparse_scaled_dot_product_attention(*args, **kwargs):
    ...
    if ATTN == "xformers":
        ...
        k = k.unsqueeze(0)
        v = v.unsqueeze(0)
        # Ensure k, v are on same device as q (for MPS GPU acceleration)
        if k.device != q.device:
            k = k.to(device=q.device, dtype=q.dtype)
            v = v.to(device=q.device, dtype=q.dtype)
        out = masked_sdpa(q, k, v, q_seqlen, kv_seqlen)
    else:
        raise ValueError(f"Unknown attention module: {ATTN}")
