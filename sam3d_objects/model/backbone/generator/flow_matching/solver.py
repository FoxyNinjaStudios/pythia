...
def linear_approximation_step(x_t, dt, velocity):
    ...
    # Ensure v is on same device as x for MPS GPU acceleration
    def step_fn(x, v):
        if hasattr(x, 'device') and hasattr(v, 'device') and x.device != v.device:
            v = v.to(device=x.device, dtype=x.dtype)
        return x + v * dt
    x_tp1 = tree_tensor_map(step_fn, x_t, velocity)
    return x_tp1
...
class ODESolver:
    ...
    def solve_iter(self, dynamics_fn, x_init, times, *args, show_progress=True, **kwargs):
        x_t = x_init
        steps = list(zip(times[:-1], times[1:]))
        if show_progress:
            steps = tqdm(steps, desc="Diffusion steps", leave=False)
        for t0, t1 in steps:
            dt = t1 - t0
